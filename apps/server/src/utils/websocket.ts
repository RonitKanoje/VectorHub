...
type WebSocketMessage = WebSocket.RawData | string; //  WebSocket.RawData can be buffer,ArrayBuffer
...
export function initWebSocket(server: Server): WebSocketServer {
  ...
  wss.on("connection", (ws) => {
    ...
    async function transcribeSegment(buffer: WebSocket.RawData): Promise<void> {
      inFlightCount++;
      ...
      try {
        await fsPromises.writeFile(tempFilePath, toWritableData(buffer));
    ...
    ws.on("message", async (message: WebSocketMessage) => {
...
): Parameters<typeof fsPromises.writeFile>[1] {
